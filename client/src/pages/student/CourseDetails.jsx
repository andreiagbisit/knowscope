...
const CourseDetails = () => {
  ...
  const enrollCourse = async () => {
    try {
      if (!userData) {
        return toast.warn ('Please sign in to enroll.')
      }
  ...
  return courseData ? (
    <>
      <div className='flex lg:flex-row flex-col-reverse gap-10 relative items-start justify-between lg:px-20 xl:px-36 px-8 md:pt-20 pt-10 text-left isolate'>
        ...
        {/* LEFT COLUMN */}
        <div className='max-w-xl z-10 text-zinc-600'>
          <h1 className='md:text-course-details-heading-large text-course-details-heading-small font-semibold text-zinc-800'>
            {courseData.courseTitle}
          </h1>
          
          <p className='pt-4 md:text-base text-sm' 
             dangerouslySetInnerHTML={{__html: SliceDesc(courseData.courseDescription, 200)}}>
          </p>
          ...
          <p className='text-sm'>
            Course by <span className='text-green-600 font-semibold'>{courseData.educator.name}</span>
          </p>
          ...
          <div className='pt-8 text-zinc-800'>
            ...
            <div className='pt-5'>
              {courseData.courseContent.map((chapter, index) => (
                <div key={index} 
                     className='border border-zinc-300 bg-white mb-2 rounded-md'>
          ...
          <div className='py-20 text-sm md:text-default'>
            ...
            <p className='pt-3 rich-text text-zinc-600!' 
        ...
        {/* RIGHT COLUMN */}
        <div className='max-w-(--course-card) z-10 shadow-custom-card rounded-2xl overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
          {
            playerData ?
          ...
          <div className='p-5'>
            ...
            <div className='flex items-center text-sm md:text-default gap-4 pt-2 md:pt-4 text-zinc-600'>
              <div className='flex items-center gap-1'>
                <img src={assets.star} alt="star icon" />
                <p>{calculateRating(courseData)}</p>
              </div>

              <div className='h-4 w-px bg-zinc-500/40 ml-4'></div>
              ...
              <div className='h-4 w-px bg-zinc-500/40'></div>

              <div className='flex items-center gap-1'>
                <img src={assets.lesson_icon} alt="clock icon" />
                <p>{calculateNoOfLectures(courseData)} {calculateNoOfLectures(courseData) === 1 ? 'lesson' : 'lessons'}</p>
              </div>
            </div>

            <button onClick={enrollCourse} 
                    className='button-custom rounded-lg md:mt-6 mt-4 w-full py-3'>
              {isAlreadyEnrolled ? 'Already Enrolled' : 'Enroll Now'}
            </button>

            <div className='pt-6'>
              <p className='md:text-xl text-lg font-medium text-zinc-800'>
                What's covered in this course?
              </p>

              <ul className='ml-4 pt-2 text-sm md:text-default list-disc text-zinc-600'>
                <li>Lifetime access with future updates.</li>
                <li>Step-by-step guided lessons.</li>
                <li>Practical, hands-on examples.</li>
                <li>Structured content for steady progress.</li>
                <li>Beginner-friendly and easy to follow.</li>
              </ul>
            </div>
          </div>
        </div>
